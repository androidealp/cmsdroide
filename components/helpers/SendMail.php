...
class SendMail extends Component{
 ...
 public $data = [];
 public $layoutSend = '@app/mail/layouts/default_mail'; //
 public $sendto = '';
 public $from = '';
 public $mensagem = '';
  ...
  */
  ...
  /**
   * Envia uma mensagem para multiplos usuários
   * @author André Luiz Pereira <andre@next4.com.br>
   * @param array $ListSend - Lista de emails e variaveis ex: [ ['email'=>'sdfs', 'var1'=>'tesdfs sdfsd'], ['email'=>'sdfs', 'var1'=>'tesdfs sdfsd']]
   * @param string $mensagem - mensagem que será enviada para o administrador
   * @return bool - retorna se o envio foi bem sucedido ou não
   */
  ...
  {
    $mailList = [];
    $usersSend = \app\_adm\models\AdmUsuarios::find()->
                                      select(['email','grupos_id'])->
                                      joinWith(['grupos'=>function($q){
                                        return $q->select(['id']);
                                      }])->
                                      where(['status_acesso'=>1])->
                                      andWhere(['like','{{%adm_grupos}}.atrib_permissoes',$type])->
                                      asArray()->All();
